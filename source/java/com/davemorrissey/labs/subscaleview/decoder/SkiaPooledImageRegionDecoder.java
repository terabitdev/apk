...
public class SkiaPooledImageRegionDecoder implements ImageRegionDecoder {
    ...
    public boolean allowAdditionalDecoder(int i, long j) {
        ...
        if (i >= getNumberOfCores()) {
            debug(i6.p(new StringBuilder("No additional encoders allowed, limited by CPU cores ("), ")", getNumberOfCores()));
            return false;
        }
        if (isLowMemory()) {
        ...
        debug(m.j(j10 / 1048576, "Mb", m.p(i, "Additional decoder allowed, current count is ", ", estimated native memory ")));
        return true;
    }
