...
public final class MaterialShapes {
    ...
    @NonNull
    @RestrictTo({RestrictTo.Scope.LIBRARY_GROUP})
    public static Matrix createRotationMatrix(float f7) {
    ...
    @NonNull
    @RestrictTo({RestrictTo.Scope.LIBRARY_GROUP})
    public static Matrix createScaleMatrix(float f7, float f10) {
        Matrix matrix = new Matrix();
    ...
    @NonNull
    @RestrictTo({RestrictTo.Scope.LIBRARY_GROUP})
    public static Matrix createSkewMatrix(float f7, float f10) {
        Matrix matrix = new Matrix();
        matrix.setSkew(f7, f10);
        return matrix;
    }

    @NonNull
    private static RoundedPolygon customPolygon(@NonNull List<VertexAndRounding> list, int i, float f7, float f10, boolean z10) {
        ArrayList arrayList = new ArrayList();
        repeatAroundCenter(list, arrayList, i, f7, f10, z10);
        return RoundedPolygonKt.RoundedPolygon(toVerticesXyArray(arrayList), CornerRounding.Unrounded, toRoundingsList(arrayList), f7, f10);
    }
    ...
    @NonNull
    private static RoundedPolygon getTriangle(float f7) {
        return Shapes_androidKt.transformed(getTriangle(), createRotationMatrix(f7));
    }
    ...
    private static void repeatAroundCenter(@NonNull List<VertexAndRounding> list, @NonNull List<VertexAndRounding> list2, int i, float f7, float f10, boolean z10) {
        boolean z11;
        int i3;
        float f11;
        list2.clear();
        toRadial(list, f7, f10);
        ...
        if (z10) {
            ...
            for (int i10 = 0; i10 < i8; i10++) {
                for (int i11 = 0; i11 < list.size(); i11++) {
                    ...
                    VertexAndRounding vertexAndRounding = list.get(i3);
                    if (i3 > 0 || !z11) {
                        float f14 = i10 * f13;
                        if (z11) {
                            f11 = (list.get(0).vertex.x * 2.0f) + (f13 - vertexAndRounding.vertex.x);
                        } else {
                            f11 = vertexAndRounding.vertex.x;
                        }
                        list2.add(new VertexAndRounding(new PointF(f14 + f11, vertexAndRounding.vertex.y), vertexAndRounding.rounding));
                    }
                }
            }
        } else {
            for (int i12 = 0; i12 < i; i12++) {
                for (VertexAndRounding vertexAndRounding2 : list) {
                    list2.add(new VertexAndRounding(new PointF((i12 * f12) + vertexAndRounding2.vertex.x, vertexAndRounding2.vertex.y), vertexAndRounding2.rounding));
                }
            }
        }
        toCartesian(list2, f7, f10);
    }

    private static void toCartesian(@NonNull List<VertexAndRounding> list, float f7, float f10) {
        Iterator<VertexAndRounding> it = list.iterator();
        while (it.hasNext()) {
            it.next().toCartesian(f7, f10);
        }
    }

    private static void toRadial(@NonNull List<VertexAndRounding> list, float f7, float f10) {
        Iterator<VertexAndRounding> it = list.iterator();
        while (it.hasNext()) {
            it.next().toRadial(f7, f10);
        }
    }
    ...
    private static float[] toVerticesXyArray(@NonNull List<VertexAndRounding> list) {
        ...
        for (int i = 0; i < list.size(); i++) {
            int i3 = i * 2;
            fArr[i3] = list.get(i).vertex.x;
            fArr[i3 + 1] = list.get(i).vertex.y;
        }
        return fArr;
    }
    ...
    public static class VertexAndRounding {
        ...
        public void toCartesian(float f7, float f10) {
            PointF pointF = this.vertex;
            float cos = (float) ((Math.cos(pointF.x) * pointF.y) + f7);
        ...
        /* JADX INFO: Access modifiers changed from: private */
        public void toRadial(float f7, float f10) {
            this.vertex.offset(-f7, -f10);
            PointF pointF = this.vertex;
    ...
    @NonNull
    private static RoundedPolygon getOval(float f7) {
        return Shapes_androidKt.transformed(getOval(), createRotationMatrix(f7));
    }
    ...
    @NonNull
    private static RoundedPolygon getGem(float f7) {
        return Shapes_androidKt.transformed(getGem(), createRotationMatrix(f7));
    }
}
